...
module.exports = function() {
	...
	// Link-to helper
	var supportedAttributes = ["class", "id", "style"];
	_helpers["link-to"] = function(path, options) {
		var $ = cheerio.load("<a>");
		$("a").attr("href", path);

		var request = false;
		if(options.hash.request) {
			request = options.hash.request;
			delete options.hash.request;
		}		

		$("a").attr(options.hash);

		if(request && request.path && request.path == path) 
			$("a").addClass("active");

		$("a").html(options.fn(this));
		return $.html();
	}
